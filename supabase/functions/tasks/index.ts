    ...
    switch (req.method) {
      case 'GET': {
        if (taskId) {
          ...
          if (!task) {
            console.log(
              `Task ${taskId} not found or access denied for user ${user.id}`,
            );
          ...
          if (cfError) {
            console.error(
              `Error fetching custom fields for task ${taskId}:`,
              cfError,
            );
          ...
          return createBadRequestResponse(
            'section_id or project_id query parameter is required',
          );
        }
      }
      case 'POST': {
        ...
        if (!projectCompanyId) {
          console.error(
            `Could not determine company ID for section ${targetSectionId}`,
          );
        ...
        if (maxOrderError) {
          console.error(
            `Error fetching max order for section ${targetSectionId}:`,
        ...
        let initialNextOccurrenceDate: string | null =
          newTaskData.next_occurrence_date || null;

        if (
          newTaskData.is_recurring_definition === true &&
          ...
          try {
            ...
            if (firstOccurrence) {
              ...
              if (
                ...
                firstOccurrence > new Date(newTaskData.recurrence_end_date)
              ) {
                console.log(
                  `First occurrence (${firstOccurrence.toISOString()}) is after recurrence end date (${newTaskData.recurrence_end_date}). No initial occurrence.`,
                );
                initialNextOccurrenceDate = null;
              } else {
                initialNextOccurrenceDate = firstOccurrence.toISOString();
                console.log(
                  `Calculated initial next_occurrence_date: ${initialNextOccurrenceDate}`,
                );
              }
            } else {
              console.warn(
                `Could not calculate first occurrence for rule: ${newTaskData.recurrence_rule}. Setting next_occurrence_date to null.`,
              );
              initialNextOccurrenceDate = null;
            }
          } catch (parseError) {
            ...
            console.error(
              `Error parsing recurrence rule during task creation: ${parseErrorMessage}`,
            );
            // Return validation error if rule is invalid
            return createValidationErrorResponse({
              recurrence_rule: [
                `Invalid recurrence rule: ${parseErrorMessage}`,
              ],
            });
          }
        }
        ...
        const { data: createdTask, error: insertError } = await supabaseClient
          ...
          .insert({
            ...
            order: nextOrder,
            // Recurrence fields
            is_recurring_definition: newTaskData.is_recurring_definition ??
              false,
        ...
        if (
          ...
          typeof newTaskData.custom_fields === 'object'
        ) {
          ...
          if (defError) {
            console.error(
              'Error fetching custom field definitions for task:',
              defError.message,
            );
          } else if (definitions) {
            // deno-lint-ignore no-explicit-any
            ...
            for (const fieldName in newTaskData.custom_fields) {
              if (definitionMap.has(fieldName)) {
                ...
                console.warn(
                  `Custom field definition not found for task field: ${fieldName}`,
                );
              }
            }
            ...
            if (valuesToUpsert.length > 0) {
              ...
              if (upsertError) {
                console.error(
                  'Error upserting custom field values for task:',
                  upsertError.message,
                );
      ...
      case 'PUT': {
        ...
        // deno-lint-ignore no-explicit-any
        const projectCompanyId =
          (taskToCheck?.sections as any)?.[0]?.projects?.[0]?.company_id ??
            (taskToCheck?.sections as any)?.projects?.company_id;
        if (!projectCompanyId) {
          console.error(`Could not determine company ID for task ${taskId}`);
          return new Response(
            JSON.stringify({
              error:
                'Internal Server Error: Project/Company information not available',
            }),
            {
              status: 500,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        ...
        const isSelfServiceAssignee = taskToCheck.is_self_service &&
          taskToCheck.assigned_to_id === user.id;
        ...
        // --- Dependency Validation (Basic) ---
        if (
          updateData.depends_on_task_id &&
          updateData.depends_on_task_id === taskId
        ) {
          return createValidationErrorResponse({
        ...
        if (
          updateData.depends_on_task_id !== undefined &&
          updateData.depends_on_task_id !== taskToCheck.depends_on_task_id
        ) {
          console.log(
            `Checking for circular dependency: Task ${taskId} -> ${updateData.depends_on_task_id}`,
          );
          const { data: isCircular, error: circularCheckError } =
            await supabaseClient.rpc(
          ...
          if (isCircular === true) {
            console.warn(
              `Circular dependency detected for task ${taskId} with dependency ${updateData.depends_on_task_id}`,
            );
            return createBadRequestResponse(
              'Setting this dependency would create a circular loop.',
            );
          }
        ...
        if (updateData.status === 'Complete') {
          ...
          if (explicitDependencyId) {
            console.log(
              `Checking explicit dependency ${explicitDependencyId} for task ${taskId} completion.`,
            );
            const { data: dependencyTask, error: depError } =
              await supabaseClient
                .from('tasks')
                .select('status')
                .eq('id', explicitDependencyId)
                .single();

            if (depError) {
              console.error(
                `Error fetching dependency task ${explicitDependencyId} for task ${taskId}:`,
                depError.message,
              );
              // Don't block completion if dependency fetch fails, but log it.
            } else if (dependencyTask?.status !== 'Complete') {
              console.warn(
                `Attempted to complete task ${taskId} but its dependency ${explicitDependencyId} is not complete.`,
              );
              return createBadRequestResponse(
                `Cannot complete task: Explicit dependency task is not yet complete.`,
              );
            }
            console.log(
              `Explicit dependency ${explicitDependencyId} is complete.`,
            );
          }

          // 2. Check condition field (if present)
          if (
            taskToCheck.condition && typeof taskToCheck.condition === 'object'
          ) {
            console.log(
              `Checking condition field for task ${taskId} completion:`,
              taskToCheck.condition,
            );
            ...
            // deno-lint-ignore no-explicit-any
            const conditionDep = (taskToCheck.condition as any)
            ...
            if (conditionDep?.task_id && conditionDep?.status) {
              ...
              console.log(
                `Condition requires task ${conditionDepId} to have status ${requiredStatus}.`,
              );

              const { data: conditionDepTask, error: condDepError } =
                await supabaseClient
                  .from('tasks')
                  .select('status')
              ...
              if (condDepError) {
                console.error(
                  `Error fetching conditional dependency task ${conditionDepId} for task ${taskId}:`,
                  condDepError.message,
                );
                // Potentially block completion if conditional dependency check fails
                return createInternalServerErrorResponse(
                  `Error checking task condition dependency: ${condDepError.message}`,
                );
              } else if (conditionDepTask?.status !== requiredStatus) {
                console.warn(
                  `Attempted to complete task ${taskId} but its conditional dependency ${conditionDepId} has status ${conditionDepTask?.status} (required: ${requiredStatus}).`,
                );
                return createBadRequestResponse(
                  `Cannot complete task: Condition not met (dependency task status is '${
                    conditionDepTask?.status ?? 'unknown'
                  }').`,
                );
              }
              console.log(
                `Conditional dependency ${conditionDepId} status check passed.`,
              );
            } else {
              console.warn(
                `Task ${taskId} has an unrecognised condition structure:`,
                taskToCheck.condition,
              );
        ...
        const isRecurringDefinition = taskToCheck.is_recurring_definition ||
          allowedUpdates.is_recurring_definition === true;
        const ruleChanged = allowedUpdates.recurrence_rule !== undefined &&
          allowedUpdates.recurrence_rule !== taskToCheck.recurrence_rule;
        const endDateChanged =
          allowedUpdates.recurrence_end_date !== undefined &&
          allowedUpdates.recurrence_end_date !==
            taskToCheck.recurrence_end_date;

        if (isRecurringDefinition && (ruleChanged || endDateChanged)) {
          const newRule = allowedUpdates.recurrence_rule ??
            taskToCheck.recurrence_rule;
          const newEndDate = allowedUpdates.recurrence_end_date ??
            taskToCheck.recurrence_end_date;
            ...
            try {
              ...
              if (nextOccurrence) {
                if (newEndDate && nextOccurrence > new Date(newEndDate)) {
                  console.log(
                    `Next calculated occurrence (${nextOccurrence.toISOString()}) is after new recurrence end date (${newEndDate}). Setting next_occurrence_date to null.`,
                  );
                  allowedUpdates.next_occurrence_date = null;
                } else {
                  allowedUpdates.next_occurrence_date = nextOccurrence
                    .toISOString();
                  console.log(
                    `Recalculated next_occurrence_date: ${allowedUpdates.next_occurrence_date}`,
                  );
                }
              } else {
                console.warn(
                  `Could not calculate next occurrence for updated rule: ${newRule}. Setting next_occurrence_date to null.`,
                );
                allowedUpdates.next_occurrence_date = null;
              }
            } catch (parseError) {
              const parseErrorMessage = parseError instanceof Error
                ...
                : 'Unknown error parsing RRULE';
              console.error(
                `Error parsing recurrence rule during task update: ${parseErrorMessage}`,
              );
              return createValidationErrorResponse({
                recurrence_rule: [
                  `Invalid recurrence rule: ${parseErrorMessage}`,
        ...
        if (
          ...
          if (defError) {
            console.error(
              'Error fetching custom field definitions during task update:',
              defError.message,
            );
          } else if (definitions) {
            // deno-lint-ignore no-explicit-any
            ...
            for (const fieldName in updateData.custom_fields) {
              if (definitionMap.has(fieldName)) {
                ...
                console.warn(
                  `Custom field definition not found for task field during update: ${fieldName}`,
                );
              }
            }
            ...
            if (valuesToUpsert.length > 0) {
              ...
              if (upsertError) {
                console.error(
                  'Error upserting custom field values during task update:',
                  upsertError.message,
                );
                // Log error but don't fail the whole request
              }
      ...
      case 'DELETE': {
        ...
        if (!projectCompanyId) {
          console.error(`Could not determine company ID for task ${taskId}`);
          return createInternalServerErrorResponse(
            'Project/Company information not available',
          );
        }

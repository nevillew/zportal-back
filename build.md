...
-   **RLS (Row Level Security):**
    -   Implement RLS helper functions (`is_active_user`, `is_staff_user`, `is_member_of_company`, `has_permission`) as `SECURITY DEFINER` functions.
    -   Define and apply RLS policies to ALL relevant tables (`companies`, `projects`, `tasks`, `documents`, `meetings`, `training` related tables, `custom_field_values`, etc.) using the helper functions. Ensure policies cover SELECT, INSERT, UPDATE, DELETE.
    -   Thoroughly test RLS policies with different user roles and scenarios.
-   **Database Triggers & Functions:**
    -   Implement `updated_at` triggers for all tables (if not already handled by `moddatetime` extension).
    -   Implement trigger/function for `sections.percent_complete` calculation based on `tasks` status changes.
    -   Implement triggers for populating `audit_log` table on relevant data modifications (INSERT, UPDATE, DELETE).
...
-   **Error Handling & Logging:**
    -   Establish `background_job_failures` table.
...
-   **Storage Policies:**
    -   Define and apply strict Supabase Storage access policies for all buckets (`company_logos`, `user_avatars`, `task_attachments`, etc.), leveraging RLS helper functions where possible.

## Existing Edge Function TODOs & Enhancements

**supabase/functions/_shared/validation.ts**

-   ✅ Enhance validation helpers to support all required validation types from `plan.md` (enums, lengths, formats, custom rules).

**supabase/functions/companies/index.ts**

-   ✅ Refine permission checks using `has_permission` helper.
-   ✅ Handle specific DB errors (e.g., unique name constraint).

**supabase/functions/custom-field-definitions/index.ts**

-   ✅ Refine permission checks (Staff only).
-   ✅ Handle specific DB errors.

**supabase/functions/issues/index.ts**

-   ✅ Implement permission checks using `has_permission`.
-   ✅ Validate status, priority enums against allowed values (POST, PUT).
-   ✅ Handle specific DB errors (FK violations, etc.) (POST, PUT, DELETE).
-   Trigger notification on assignment or status change (if required by plan).

**supabase/functions/milestones/index.ts**

-   ✅ Implement permission checks using `has_permission` (CRUD, approve).
-   ✅ Validate status enum against allowed values (POST, PUT).
-   ✅ Implement full approval workflow logic:
    -   ✅ Check `sign_off_required` flag.
    -   ✅ Update status, `signed_off_by_user_id`, `signed_off_at` on approval (POST /approve).
...
-   ✅ Handle specific DB errors (unique constraints, FK violations, restricted delete) (POST, PUT, DELETE).
...
-   Implement permission checks using `has_permission`.
-   Handle specific DB errors.
-   Ensure `percent_complete` is correctly updated by the trigger/function (verify trigger implementation).
